...
export default async function Empresas({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const statusLabel: Record<EmpresaStatus, string> = {
    ...
    construcao: d.empresas.statusEmConstrucao,
    beta: d.empresas.statusBeta,
      ...
      <section className="relative px-6 md:px-[8rem] pt-24 pb-20 overflow-hidden border-b border-border">
        ...
        <div className="relative max-w-[820px]">
          <div className="flex items-center gap-3 mb-10">
            ...
            <span className="font-mono text-[10px] tracking-[0.35em] uppercase text-champagne">{d.empresas.eyebrow}</span>
          </div>
            ...
            <article key={e.nome} className={`grid grid-cols-1 md:grid-cols-2 bg-bg min-h-[360px] ${i % 2 === 1 ? 'md:[&>*:first-child]:order-2' : ''}`}>
              ...
              <div className="p-10 md:p-14 flex flex-col justify-center">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className={`font-mono text-[9px] tracking-[0.2em] uppercase px-2 py-1 rounded-full border ${e.status === 'ativa' ? 'border-champagne/40 text-champagne' : 'border-bronze/40 text-bronze'}`}>{statusLabel[e.status]}</span>
                </div>
